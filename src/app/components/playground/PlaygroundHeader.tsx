...
interface PlaygroundHeaderProps {
  ...
  onConnectAction: 'connect' | 'disconnect';
}
...
export const PlaygroundHeader = ({
  ...
  const handleConnect = useCallback(() => {
    if (connectionState === ConnectionState.Connected) {
      window.dispatchEvent(new CustomEvent('playground:disconnect'));
    } else {
      window.dispatchEvent(new CustomEvent('playground:connect'));
    }
  }, [connectionState]);
    ...
    >
      <div className="flex items-center gap-3 basis-2/3">
        <div className="flex lg:basis-1/2">
          <a href="https://livekit.io">{logo}</a>
        </div>
      ...
      <div className="flex basis-1/3 justify-end items-center gap-2">
        ...
        <Button
          ...
          className={`bg-${connectionState === ConnectionState.Connected ? "red" : accentColor}-500`}
